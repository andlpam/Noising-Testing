...
#DA3 BUGS
RUN sed -i 's/reference_view_strategy=reference_view_strategy/reference_view_strategy=ref_view_strategy/g' /app/src/depth_anything_3/cli.py && \
...
#Copy all files just for simplicity
COPY *.py /app/

#Use entrypoint to pass the parameters directly..
ENTRYPOINT ["python", "run_reconstructions.py"]
